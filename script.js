...
const searchbutt = document.querySelector(".serrr");
searchbutt.addEventListener("click", async function () {
  const inpkeyword = document.querySelector(".input-key");
  const movieee = await getMovies(inpkeyword.value);
  updateUI(movieee);
});

//event bidding
document.addEventListener("click", async function (e) {
  if (e.target.classList.contains("modal-detail-b")) {
    const imdbid = e.target.dataset.imdbid;
    const modetil = await getdetailMovies(imdbid);
    updateUIDeail(modetil);
  }
});

function getdetailMovies(imdbid) {
  return fetch("http://www.omdbapi.com/?apikey=f54d78a9&i=" + imdbid)
    .then((response) => response.json())
    .then((m) => m);
}

function updateUIDeail(m) {
  const movidetal = showsie(m);
  const modal_body = document.querySelector(".modal-body");
  modal_body.innerHTML = movidetal;
}
...
function getMovies(keyword) {
  return fetch("http://www.omdbapi.com/?apikey=f54d78a9&s=" + keyword)
    .then((response) => response.json())
